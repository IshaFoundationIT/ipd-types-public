...
export type TranslatableText = { lang: string; text: string };
export type NumberPerLang = { lang: string; value: number };
export type CreatePayload<T extends { id: unknown; createdAt: unknown; updatedAt: unknown }> = Omit<T, "id" | "createdAt" | "updatedAt">;
export type UpdatePayload<T extends { id: unknown }> = Pick<T, "id"> & Partial<Omit<T, "id">>;
...
export interface ListAPIResponse<T = unknown>
  extends IAPIResponse<{ rows: T[]; count: number }> {}
